...
def merge(pairs_path, output, nproc):
    """merge sorted pairs/pairsam files. 

    Merge triu-flipped sorted .pairs/.pairsam files. If present, the @SQ records 
    of the SAM headers of the merged files must be identical; the sorting order of 
    ...
    """
    merge_py(pairs_path, output, nproc)


def merge_py(pairs_path, output, nproc):
