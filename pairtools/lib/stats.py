...
class PairCounter(Mapping):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.single_mapped_by_side = single_mapped_by_side
        ...
        for key in self.filters:
            ...
            if single_mapped_by_side:
                self._stat[key]["total_left_only_mapped"] = 0
                self._stat[key]["total_right_only_mapped"] = 0
    ...
    def add_pair(
        ...
        if chrom1 == unmapped_chrom and chrom2 == unmapped_chrom:
            ...
            if self.single_mapped_by_side:
                if chrom1 == unmapped_chrom:
                    self._stat[filter]["total_right_only_mapped"] += 1
                else:
                    self._stat[filter]["total_left_only_mapped"] += 1
    ...
    def add_pairs_from_dataframe(self, df, unmapped_chrom="!"):
        ...
        for key in self.filters.keys():
            ...
            if self.single_mapped_by_side:
                ...
                right_only_mapped_count = np.logical_and(
                    df_filtered["chrom1"] == unmapped_chrom,
                    df_filtered["chrom2"] != unmapped_chrom,
                ).sum()
                self._stat[key]["total_left_only_mapped"] += int(left_only_mapped_count)
        ...
        for k, v in self._stat[filter].items():
            ...
            # nested categories are stored only if they are non-trivial
                ...
                if (k == "dist_freq") and v:
                    ...
                    print(
                        f"Can't flatten {k} field: not a trivial field, not implemented"
                    )
    ...
    def format_yaml(self, filter="no_filter"):
        ...
        for filter_name in self.filters.keys():
            for k, v in self._stat[filter_name].items():
                ...
                if v is not None: # skip empty fields
